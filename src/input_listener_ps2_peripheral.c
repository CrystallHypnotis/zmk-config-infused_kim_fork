 ...
 *
 * Peripheral-side PS/2 input listener.
 *
 * Modified from original input_listener_ps2.c:
 * - Instead of calling zmk_hid_mouse_* and zmk_endpoints_send_mouse_report()
 *   (which only work on central), this listener re-injects accumulated
 *   mouse events into the Zephyr input subsystem via input_report().
 * - ZMK_INPUT_SPLIT (from urob's fork) forwards these events to the central.
 * - Layer toggle (auto-activate on mouse movement) still works on peripheral.
 */

#define DT_DRV_COMPAT zmk_input_listener_ps2_peripheral
...
struct input_listener_ps2_data {
    ...
    // Recursion guard: prevents re-entry when we call input_report()
    // from within our own input handler callback.
    bool re_injecting;
};
...
static void zmk_input_listener_ps2_layer_toggle_input_rel_received(
...
static void zmk_input_listener_ps2_layer_toggle_activate_layer(struct k_work *item) {
...
static void zmk_input_listener_ps2_layer_toggle_deactivate_layer(struct k_work *item) {
    struct k_work_delayable *d_work = k_work_delayable_from_work(item);
    struct input_listener_ps2_data *data =
...
static int zmk_input_listener_ps2_layer_toggle_init(
    const struct input_listener_ps2_config *config, struct input_listener_ps2_data *data) {
...
static void input_handler_ps2_peripheral(const struct input_listener_ps2_config *config,
                                         struct input_listener_ps2_data *data,
                                         struct input_event *evt) {
    // Re-entry guard: when we call input_report() below to re-inject
    // accumulated events, our own callback gets triggered again.
    // Skip processing during re-injection.
    if (data->re_injecting) {
        return;
    }
    ...
    if (evt->sync) {
        // Re-inject accumulated movement as input events.
        // ZMK_INPUT_SPLIT will forward these to the central.
        // Set re-entry guard to prevent our own callbacks from
        // processing the re-injected events.
        data->re_injecting = true;

        if (data->mouse.data.mode == INPUT_LISTENER_XY_DATA_MODE_REL) {
            input_report(evt->dev, INPUT_EV_REL, INPUT_REL_X,
                         data->mouse.data.x, false, K_NO_WAIT);
            input_report(evt->dev, INPUT_EV_REL, INPUT_REL_Y,
                         data->mouse.data.y, false, K_NO_WAIT);
        }

        if (data->mouse.wheel_data.mode == INPUT_LISTENER_XY_DATA_MODE_REL) {
            if (data->mouse.wheel_data.y != 0) {
                input_report(evt->dev, INPUT_EV_REL, INPUT_REL_WHEEL,
                             data->mouse.wheel_data.y, false, K_NO_WAIT);
            }
            if (data->mouse.wheel_data.x != 0) {
                input_report(evt->dev, INPUT_EV_REL, INPUT_REL_HWHEEL,
                             data->mouse.wheel_data.x, false, K_NO_WAIT);
            }
        }

        // Re-inject accumulated button state changes
        for (int i = 0; i < 5; i++) {
            if ((data->mouse.button_set & BIT(i)) != 0) {
                input_report(evt->dev, INPUT_EV_KEY, INPUT_BTN_0 + i, 1, false, K_NO_WAIT);
            }
            if ((data->mouse.button_clear & BIT(i)) != 0) {
                input_report(evt->dev, INPUT_EV_KEY, INPUT_BTN_0 + i, 0, false, K_NO_WAIT);
            }
        }

        // Send sync to flush
        input_report(evt->dev, INPUT_EV_SYN, 0, 0, true, K_NO_WAIT);

        data->re_injecting = false;
...
#define IL_INST(n)                                                                                 \
    COND_CODE_1(                                                                                   \
        ...
        (                                                                                          \
            ...
            void input_handler_ps2_peripheral_##n(struct input_event *evt) {                       \
                input_handler_ps2_peripheral(&config_##n, &data_##n, evt);                         \
            }                                                                                      \
            INPUT_CALLBACK_DEFINE(DEVICE_DT_GET(DT_INST_PHANDLE(n, device)),                       \
                                  input_handler_ps2_peripheral_##n);                               \
            static int zmk_input_listener_ps2_peripheral_init_##n(const struct device *dev) {      \
            ...
            DEVICE_DT_INST_DEFINE(n, &zmk_input_listener_ps2_peripheral_init_##n,                  \
